...
def find_n_list(n,domain,nbr_samples,f,tol,max_nbr_intervals=100):
    list_n=[]
    list_intervals=[]
    d=domain.shape[0]
    for axis in range(d):
        root=IntervalTree(domain[axis, 0], domain[axis, 1])
        counter = [1]  # root domain counts as 1 interval initially
        find_subinterval_d(domain, n, axis, nbr_samples, f, root, max_nbr_intervals=max_nbr_intervals, counter=counter,tol=tol)
...
def adaptive_tt_offline(n,domain,nbr_samples,f,tol,max_nbr_intervals=100):

    list_n,list_intervals=find_n_list(n,domain,nbr_samples,f,tol=tol,max_nbr_intervals=max_nbr_intervals)
...
def adaptive_tt_online(n,domain,tt_cores,list_n,list_intervals,x_points,y_points,d,N_s,N_t):
    D=domain.shape[0]
