...
def PTTK_offline(f,domain,n,x_points,y_points,d,d_theta,N_s,N_t,tol=1e-6):
   ...
   # cores,cores_shape,ranks=tt_svd_delta(M,eps=tol)
    ...
    S=U[0]@vertical_core(tt.cores[0])
    for i in range(1,d):
        G_i2=vertical_core(tt.cores[i])
        S = khatri_rao(S.T, (U[i]).T).T@G_i2

    T=horizontal_core(tt.cores[D-1])@V[d-1].T
    for i in range(1,d):
        T=horizontal_core(tt.cores[D-1-i])@khatri_rao(V[d-1-i].T, T)
    T=T.T
    ...
    for j in range(d_theta):
        cores_approx.append(tt.cores[d+j])

    cores_approx.append(T_twisted)
    tt_approx=TT(cores_approx)
    tt_afterrounding,ranks_rounding=tt_approx.rounding(eps=1e-6)
    t4=time.perf_counter()
    print("time for stage 4 is :"+str(t4-t3)+" seconds")
    print("-------------------")
    print("total time is :"+str(t4-t0)+" seconds")

    return tt_afterrounding, ranks_rounding
   
def PTTK_online(theta, d_theta,d,domain,n,tt_afterrounding,ranks_rounding):
    cheb_nodes = cheb.chebpts1(n)
    ...
    for j in range (d_theta):
        ...
        H_j=np.tensordot(tt_afterrounding.cores[j+1],qp_j,axes=([1],[0]))#bc we sum over the second dimension of the core and the first dimension of qp_j
        if j==0:
            H=H_j
        else:
            H=H@H_j

    K=tt_afterrounding.cores[0].reshape((-1,ranks_rounding[1]))@H@tt_afterrounding.cores[-1].reshape((ranks_rounding[-2],-1))
