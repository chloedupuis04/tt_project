...
def find_subinterval_d(domain, n, axis, nbr_of_samples, f, node, max_nbr_intervals, counter, tol=1e-6):
    """
    counter: a mutable list, e.g. [1], holding the current total number of intervals created.
             Initialize with counter=[1] on the top-level call (root counts as 1 interval).
    """
    resolved = []
    d = domain.shape[0]
    ...
    for j in range(nbr_of_samples):
        ...
        reference_nodes = cheb.chebpts2(n)
        local_nodes = phi(a, b, reference_nodes)
        function_values = function_1d(local_nodes)
        coefficients = cheb.chebfit(reference_nodes, function_values, deg=n - 1)
        p = cheb.Chebyshev(coefficients, domain=[a, b])
        f_values = function_1d(values_test)
        p_values = p(values_test)
        max_error = np.max(np.abs(f_values - p_values))
        resolved.append(max_error < tol)
    ...
    if counter[0] + 2 > max_nbr_intervals:
        print("max intervals reached")
        return 
    ...
    # Splitting one interval into two adds 1 net new interval (1 removed, 2 added)
    counter[0] += 1

    find_subinterval_d(left_domain, n, axis, nbr_of_samples, f, left_node, max_nbr_intervals, counter, tol=tol)
    find_subinterval_d(right_domain, n, axis, nbr_of_samples, f, right_node, max_nbr_intervals, counter, tol=tol)